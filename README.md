...
2. What is an event?
Typical event is an object based on the `Event` interface, representing an action that takes place in the DOM. Events can be user-generated (i.e. keyboard events or mouse events such as scroll or DOM element focus) or generated by the numerous APIs (i.e. that animation has finished running).
...
We can convert NodeList into an Array if we want to use the methods that NodeList lacks of and the Array has. Especially if we want to use the non-default iteration methods or array methods (such as map or reduce; at the same time, it's worth noticing that the forEach method is available for NodeList).
